...
import { History as HistoryIcon, Server, Database, ChevronLeft, ChevronRight, Zap, RefreshCcw, Loader2 } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <div className="flex items-center gap-4">
                ...
                <button onClick={fetchLogs} disabled={isRefreshing} className="p-2 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all active:scale-95 border border-slate-200">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                              ...
                              currentLogs.map((item) => (
                                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                      <td className="p-4 text-center">
                                          <div className={`inline-flex p-2 rounded-lg ${item.source === 'API' ? 'bg-amber-100 text-amber-600' : 'bg-green-100 text-green-600'}`}>{item.source === 'API' ? <Server size={16} /> : <Database size={16} />}</div>
                                      </td>
                                      ...
                                      <td className="p-4">{item.source === 'API' && item.tokens ? <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-600 border border-slate-200"><Zap size={10} className="fill-current" /> {item.tokens}</span> : <span className="text-slate-400 text-xs">-</span>}</td>
                                      <td className="p-4 text-right"><span className="text-slate-500 text-xs md:text-sm font-mono">{new Date(item.timestamp).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span></td>
                                  </tr>
                              ))
                          )}
                      </tbody>
            ...
            {!loading && logs.length > 0 && (
                <div className="p-4 border-t border-slate-200 flex items-center justify-between bg-slate-50/50">
                    <p className="text-sm text-slate-500"><span className="font-medium">{((currentPage - 1) * ITEMS_PER_PAGE) + 1}</span> - <span className="font-medium">{Math.min(currentPage * ITEMS_PER_PAGE, logs.length)}</span> dari {logs.length}</p>
