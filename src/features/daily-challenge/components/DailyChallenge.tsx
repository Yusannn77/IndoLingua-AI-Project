        ...
        <div className="animate-slide-up space-y-6">
          {!scenario ? (
            ...
            <div className="space-y-4">
                <div className="bg-white border-2 border-amber-100 p-6 rounded-3xl relative overflow-hidden"><div className="absolute top-0 left-0 w-full h-2 bg-amber-400"></div><div className="text-xs font-extrabold bg-amber-100 text-amber-800 px-3 py-1 rounded-full inline-block mb-4">TARGET: {scenario.word}</div><h3 className="text-2xl font-medium text-slate-900">&quot;{scenario.situation}&quot;</h3></div>
                {!feedback ? (
                    <>
                    <textarea value={response} onChange={e => setResponse(e.target.value)} className="w-full p-6 border border-slate-300 rounded-2xl min-h-[120px] text-lg" placeholder="Jawab di sini..." />
                    <button onClick={submitMission} disabled={loadingAI} className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 disabled:opacity-70">{loadingAI ? <span className="flex items-center justify-center gap-2"><RefreshCcw className="animate-spin"/> Menilai...</span> : 'Kirim'}</button>
                    </>
                ) : (
                    <div className={`p-6 rounded-3xl border-2 ${feedback.score >= 5 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
                        ...
                        <div className="flex gap-3"><button onClick={() => setFeedback(null)} className="flex-1 py-3 bg-white border border-slate-300 text-slate-700 rounded-xl font-bold hover:bg-slate-50">Ulangi</button><button onClick={startMission} className="flex-1 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-black shadow-lg">Lanjut</button></div>
                    </div>
                )}
            </div>
          )}
